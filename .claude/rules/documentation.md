...
These rules govern what "update documentation" (root `CLAUDE.md` → Development
workflow, step 7) actually requires. They apply whenever a change alters observable
behavior — a new/changed API contract, a new/changed data model, a new user flow, or
...
Use Context7 before implementing framework-specific behavior. 
Prefer official, version-specific documentation.
 Do not rely only on remembered framework APIs.