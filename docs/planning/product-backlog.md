...
8. **Database impact:** New table `tenant` (id UUID PK, name, subdomain UNIQUE NOT NULL, status CHECK-constrained enum `pending_approval/trial/active/suspended/cancelled/rejected`, plan_id, created_at, updated_at). Root platform table — coordinate with APP-4 to avoid a duplicate migration. **Note (resolved 2026-08-08):** the original 4-value enum listed here (`trial/active/suspended/cancelled`) could not represent this story's own "pending-approval status" requirement or TEN-2's reject transition; `pending_approval` and `rejected` were added to close that gap. See `docs/plans/MVP-004 Tenant Management.md` §7/§21 item 1 for the full transition graph and rationale.
