...
| PAR-26-04 | Teacher / Tenant Admin | Course-level review enable/disable toggle | Still not built — the tenant-configuration framework it depends on (PAR-XC-02) now exists, but `COURSE`/course-level config has no registered properties yet (Wave 1 only populated `GENERAL`/`BRANDING`) and no course-level (as opposed to tenant-level) override mechanism was built this wave | course-level config | MISSING_CONFIGURATION | Framework prerequisite (PAR-XC-02) is satisfied; the toggle itself is still Wave 2 work | Wave 2 |
